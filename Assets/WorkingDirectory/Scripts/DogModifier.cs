...
public class DogModifier : MonoBehaviour
{
    ...
    [SerializeField] public int numberOfParts = 8;
    ...
    {
        partChosen = "Body";
        parts = new string[] { "Body", "Back", "Ears", "Eyes", "Mouth", "Nose", "Pawpads", "Tail"}; //Keep these the same order as the buttons that appear in the unity editor
        colors = new string[] { "wht","blu","red","mar","ylw","grn","fus","blk"}; //These must match the below color codes
    ...
    public void UpdateModel()
    {
        ...
        foreach (int i in materials) //Applying Textures
        {
        ...
        foreach (int i in materials) //Applying Colors
        {
        ...
        tracker.recordAction("HOLO-DOG," + partChosen + "," + colorHexaCodes[getColorIndex()] + "," + texturesChosen[partIndex]);
    }
}
